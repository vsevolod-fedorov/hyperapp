...
def test_type_resolver(ref_resolver, type_resolver, ref_registry):
    ...
    assert (resolve_1('record_1').match(TRecord([Field('int_field', tInt)])))

    assert resolve_1('some_int') is tInt

    assert resolve_1('record_1').match(TRecord([Field('int_field', tInt)]))
    assert resolve_1('record_2').match(TRecord([Field('int_field', tInt), Field('string_field', tString)]))

    object_t = resolve_1('object')
    simple_class = resolve_1('simple_class')

    assert simple_class.hierarchy is object_t
    assert simple_class.match(TClass(object_t, 'simple_2', TRecord([])))

    assert resolve_1('text_object').match(
        TClass(object_t, 'text_2', base=simple_class, trec=TRecord([Field('text', tString)])))
