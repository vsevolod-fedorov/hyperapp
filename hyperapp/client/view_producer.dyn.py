...
class ViewProducer:
    ...
    async def produce_view(self, state, object, observer=None):
        ...
        resource = self._resource_resolver.resolve(resource_key, LOCALE)
        if resource:
            producer = self._layout_registry.resolve(resource)
            _log.info("Producing view for %s %r with %s using %s", ref_repr(type_ref), state, resource, producer)
