...
class EncryptedTransport(Transport):
    ...
    async def _process_subsequent_encrypted_packet(self, server_public_key, session, encrypted_packet):
        ...
        pprint(packet.aux_info, self._resource_types, self._error_types, self._packet_types, self._iface_registry)
        await self.process_aux_info(packet.aux_info)
        pprint(packet.payload, self._resource_types, self._error_types, self._packet_types, self._iface_registry)
        return ResponseBase.from_data(self._error_types, self._packet_types, self._iface_registry, server_public_key, packet.payload)
