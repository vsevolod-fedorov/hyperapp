...
def pick_refs(pick_refs_cache, value, t=None):
    try:
        return pick_refs_cache.value_to_refs[value]
    except KeyError:
        pass
    if t is None:
        t = deduce_t(value)
    try:
        picker = pick_refs_cache.t_to_picker[t]
    except KeyError:
        picker = pick_refs_cache.t_to_picker[t] = _type_to_picker(t)
    refs = list(picker.pick_refs(value))
    pick_refs_cache.value_to_refs[value] = refs
    return refs
