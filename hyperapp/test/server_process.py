...
def sync_fn_to_async_future(event_loop, thread_pool, get_result_fn, timeout_sec=3):
    async_future = event_loop.create_future()
    def handle_result():
        log.debug('sync_future_to_async_future.handle_result: started')
        try:
            result = get_result_fn(timeout=timeout_sec)
            log.debug('sync_future_to_async_future.handle_result: result=%r', result)
            event_loop.call_soon_threadsafe(async_future.set_result, result)
            log.debug('sync_future_to_async_future.handle_result: succeeded')
        except Exception as x:
            log.exception('sync_future_to_async_future.handle_result:')
            event_loop.call_soon_threadsafe(async_future.set_exception, x)
    thread_pool.submit(handle_result)
    log.debug('sync_future_to_async_future: ready')
    return async_future

def sync_future_to_async_future(event_loop, thread_pool, mp_future, timeout_sec=3):
    return sync_fn_to_async_future(event_loop, thread_pool, mp_future.result, timeout_sec)
