        ...
        for dir in self._dir_list:
            for path in dir.rglob('*.dyn.py'):
                ...
                if name_parts[-1] == 'fixtures':
                    unit = FixturesUnit(graph, ctx, self._generator_ref, self._root_dir, path)
                elif name_parts[-1] == 'tests':
                    unit = TestsUnit(graph, ctx, self._generator_ref, self._root_dir, path)
                else:
                    unit = Unit(graph, ctx, self._generator_ref, self._root_dir, path)
                unit.init()
