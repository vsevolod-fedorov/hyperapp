...
class TreeView(View, QtWidgets.QTreeView, TreeObserver):
    ...
    def state(self):
        ...
        return self._object.make_state(current_key=self.current_item_key)

    @state.setter
    def state(self, state):
        if not state:
            return
        self._wanted_current_path = state.current_key  # Will set it to current when rows are loaded.
    ...
    def _on_data_changed(self):
        ...
        if self.model().rowCount(QtCore.QModelIndex()) == 1:
            # Expand first row it it is the only one.
...
class ThisModule(Module):
    ...
    def __init__(self, module_name, services, config):
        ...
        services.lcs.set([htypes.view.view_d('default'), *TreeObject.dir_list[-1]], htypes.tree_view.tree_view())
        services.lcs.add(
