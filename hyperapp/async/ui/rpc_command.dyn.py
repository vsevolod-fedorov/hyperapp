...
def element_command_interface_ref(mosaic, key_type_ref, method_name):
    item_key_field = field_mt('item_key', key_type_ref)
    method_ref = mosaic.put(request_mt(method_name, [item_key_field], []))
...
class RpcElementCommand:
    ...
    @classmethod
    async def from_piece(cls, piece, command_id, mosaic, rpc_endpoint, async_rpc_proxy, identity):
        interface_ref = element_command_interface_ref(mosaic, piece.key_type_ref, piece.method_name)
        ...
        return cls(command_id, piece.key_type_ref, piece.method_name, piece.peer_ref, piece.object_id, proxy)

    def __init__(self, id, key_type_ref, method_name, peer_ref, object_id, proxy):
        self.id = id
...
class ThisModule(Module):
    ...
    async def async_init(self, services):
        services.command_registry.register_actor(
            ...
            services.mosaic, services.client_rpc_endpoint, services.async_rpc_proxy, services.client_identity)
