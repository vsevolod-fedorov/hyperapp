...
class TreeService(TreeObject):
    ...
    async def from_piece(
        ...
        return cls(mosaic, types, peer, piece.servant_fn_ref, rpc_call, dir_list, command_list, piece.key_attribute)

    def __init__(self, mosaic, types, peer, servant_fn_ref, rpc_call, custom_dir_list, command_list, key_attribute):
        super().__init__()
        self._mosaic = mosaic
        self._types = types
        self._peer = peer
    ...
    @property
    def title(self):
        return f"Tree service: {self._servant_fn_ref}"
